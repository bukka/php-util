...
function phpu_conf {
  ...
  if [[ $PHPU_CURRENT_DIR =~ ^(src|std|sec|master|7|71|72|73|74)$ ]]; then
    if [[ ! "$*" =~ "no-debug" ]]; then
  ...
  elif [[ $PHPU_CURRENT_DIR =~ ^(7|71|72|73|74)$ ]]; then
    PHPU_CONF_ACTIVE_EXT="$PHPU_CONF_EXT_MASTER"
...
function phpu_use {
  ...
  if [ -n "$1" ]; then
    ...
    elif [[ "$1" =~ ^(master|7|71|72|73|74)$ ]]; then
      if [[ "$1" == "7" ]]; then
        cd "$PHPU_7"
      elif [[ "$1" == "71" ]]; then
        cd "$PHPU_71"
    ...
    if $PHPU_MAKE_J && sudo make install ; then
      ...
      if ! [[ "$1" =~ ^(master|7|71|72|73|74)$ ]]; then
